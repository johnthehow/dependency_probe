...
def report_writer(probe_trained, dataset):
    ...
    report_filepath = REPORTS_PATH.joinpath(f'REPORT[COR]SPEARMANR[ACC]UUAS[MODEL]{MODEL_NAME}[LAYER]{HIDDEN_LAYER}[EPOCHS]{EPOCHS}[BATCHSIZE]{BATCH_SIZE}[RANK]{PROBE_RANK}[LR]{LEARNING_RATE}[LOSSPOW]{PROBE_LOSS_POW}[METHOD]{METHOD}[DATA]{CONLL_ALIAS}[PATHD]{str(PATH_DISTANCE).lower()}[DIRECTED]{str(DEPD_DIRECTED).lower()}.txt')
